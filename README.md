...
2. Create & provide a PAT (Personal Access Token) for the CI/CD pipeline
   ...
   1. If the new repo owner is an organisation, **enrol** the organisation for '**Fine-grained personal access tokens**'. In the organisation ['Settings > Third-party Access > Personal access tokens'](https://github.com/organizations/xdab/settings/personal-access-tokens).
...
4. Final one-off tasks
   ...
   * Update _Maven Publication_ details [here](aprs4j/build.gradle.kts#L6-L13)
...
* `aprs4j/`: The library source code (Gradle sub-project).
...
## CI/CD Pipeline
The heart of this template is the ['Main GitHub Actions CI/CD Pipeline'](https://github.com/xdab/aprs4j/actions/workflows/1.pipeline.yml).
...
Please define the secrets via your repository settings. ([Settings > Security > Secrets and variables > Actions](https://github.com/xdab/aprs4j/settings/secrets/actions))
...
To host the generated Javadoc, configure GitHub Pages for your repository to deploy from branch `gh-pages`. You can also find all deployments under ['pages-build-deployment'](https://github.com/xdab/aprs4j/actions/workflows/pages/pages-build-deployment).
...
The libraries gradle dependencies are scanned for known [CVE](https://www.cve.org/) with **[aquasecurity/trivy](https://github.com/aquasecurity/trivy)**. The scan results can be reviewed and managed under [Security > Vulnerability alerts > Code scanning](https://github.com/xdab/aprs4j/security/code-scanning).
